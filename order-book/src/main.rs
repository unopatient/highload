...
fn main() {
    ...
    // let buf_len = buf.len();
    // let str = std::str::from_utf8(buf).expect("Failed to convert.");
    // println!("{:?}", buf_len);
    // println!("{}", str);
    ...
    for &c in buf {
        ...
        match state {
            ...
            State::ParseNumA => {
                match c {
                    SPACE => {
                        // println!("Moving to ParseNumB");
                        num_b = 0;
                        state = State::ParseNumB;
                    },
                    NEWLINE => {    // Hopefully their test cases end with newlines
                        match sign {
                            EQUALS => {
                                // Remove 'num_a' liquidity 
                                // println!("= {}", num_a);
                                take_liquidity(&mut price_to_order_queue, num_a);
                            },
                            MINUS => {
                                // Remove order at the index 'num_a'
                                // println!("- {}", num_a);
            ...
            State::ParseNumB => {
                match c {
                    NEWLINE => {
                        // At 'num_a' price add order for 'num_b'
                        // println!("+ {} {}", num_a, num_b);
...
fn add_liquidity(
    ...
    // Proably better to use something arrray based isdh
...
fn remove_order(
    ...
    while index_in_current_order_queue >= current_order_queue_entry.1.len() {
        ...
        // We advance to next queue AFTER decerementing index with current queue length
...
fn take_liquidity(
    ...
    while remaining_amount > 0 {
        ...
        // first_order_queue.is_empty() {
        //     price_to_order_queue.remove(&first_order_queue_price);
        // } else if remaining_amount < front_order_amount {
        //     *first_order_queue.front_mut().unwrap() -= remaining_amount;
        //     remaining_amount = 0;
        // }


        // while !first_order_queue.is_empty() && remaining_amount >= front_order_amount {
        //     // Remove order if remaining amount is greater or equal
        //     front_order_amount = *first_order_queue.front().expect("Empty order queue");

        //     println!("Inner loop first_order_queue.len(): {}", first_order_queue.len());
        //     println!("Inner loop front_order_amount: {}", front_order_amount);
        //     println!("Inner loop pre remaining amount: {}", remaining_amount);
        //     remaining_amount -= front_order_amount;
        //     println!("Inner loop post remaining amount: {}", remaining_amount);
        //     first_order_queue.pop_front();
